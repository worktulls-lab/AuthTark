...
public final class AuthFlow {
    ...
    public static void beginProtection(AuthPlugin plugin, Player player, boolean needsRegister) {
        ...
        // Без большого титра на весь экран — только boss bar сверху и обычная строка в чат
        boolean captchaPending = needsRegister && plugin.getCaptchaManager().isRequired(player.getUniqueId());
        if (!captchaPending) {
            Msg.send(player, needsRegister
